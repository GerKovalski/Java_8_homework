...
public class Homework03 {
  ...
  public void sortedByRepeatableChars() {
    ...
    Function<String, Integer> mostlyRepeatedChar = string -> {

      HashMap<String, Integer> groupedChars = Arrays.stream(string.toLowerCase().split(""))
          .collect(HashMap::new,
              (HashMap<String, Integer> map, String str)
                  -> map.merge(str, 1, (Integer val1, Integer val2) -> val1 + val2), HashMap::putAll);

      return (int) groupedChars.values().stream().max(Integer::compareTo).orElse(-1);

    };

    String[] result = words.stream()
        // TODO: Add realization
        .sorted((w1, w2) -> mostlyRepeatedChar.apply(w2) - mostlyRepeatedChar.apply(w1))
        .toArray(String[]::new);
  ...
  public void uniqueValues() {
    ...
    int[] result = numbers.stream().distinct().mapToInt(number -> number).toArray();
