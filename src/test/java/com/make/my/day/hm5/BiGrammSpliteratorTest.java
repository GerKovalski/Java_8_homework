...
public class BiGrammSpliteratorTest {
  ...
  public void biGramSplitTest() throws Exception {
    List<String> tokens = Arrays.asList(
        "I should never try to implement my own spliterator".split(" "));
  ...
  class BigrammSpliterator implements Spliterator<String> {
     ...
     */
    public BigrammSpliterator(List<String> source, String delimiter) {
      this(source, delimiter, 0, source.size() - 1);
    }

    private BigrammSpliterator(List<String> source, String delimiter, int start, int end) {
      this.source = source;
    ...
    public boolean tryAdvance(Consumer<? super String> action) {
      ...
      action.accept(source.get(start) + delimiter + source.get(++start));
      return true;
    }
    ...
    public BigrammSpliterator trySplit() {
      ...
      BigrammSpliterator newSpliterator = new BigrammSpliterator(source, delimiter, mid, end);
      end = mid;
