...
public class Homework04 {
  ...
  public void collectToMap() {
    ...
    Map<String, Integer> result = Arrays.stream(words)
        .collect(HashMap::new,
            (map, word) -> map.merge(word, 1, (valBefore, valAdd) -> valBefore + valAdd),
            HashMap::putAll);
  ...
  public void collectingAndThen() {
    ...
    List<String> result = Arrays.stream(words)
        .collect(Collectors.collectingAndThen(
            Collectors.toSet(),
            el -> el.stream().map(s -> s.concat(s)).collect(Collectors.toList())
        ));
  ...
  public void dogMappingByAge() {
    ...
    Map<String, List<Integer>> result = dogs.stream()
        .collect(Collectors
            .groupingBy(Dog::getName, Collectors.mapping(Dog::getAge, Collectors.toList())));
  ...
  public void convertFromUserToUserDTOTest() {
    ...
    List<UserDTO> result = usersFromDB.stream().collect(Collectors.groupingBy(User::getEmail,
        Collectors.mapping(User::getRole, Collectors.toList()))).entrySet().stream()
        .map(stringListEntry ->
            new UserDTO(stringListEntry.getKey(), stringListEntry.getValue()))
