...
public class Homework02 {
  ...
  public void chainOfFunctionInvocation() {
    ...
    Function<List<Integer>, List<Integer>> sumWithNextElement = original -> {
        List<Integer> result = new ArrayList<>();
        for (int i = 0; i < original.size() - 1; i++) {
            result.add(original.get(i) + original.get(i + 1));
        }
        result.add(original.get(original.size() - 1) + original.get(0));
        return result;
    };
  ...
  private static Function<Predicate<String>, Function<String, String>> doubleStringOrAddThreeDots() {
    return predicate -> (str -> predicate.test(str) ? str + str : str + "...");
  }
